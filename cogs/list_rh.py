...
class RHView(discord.ui.View):
    ...
    def parse_data(self):
        for st in self.stations:
            ...
            rh_str = weather.get('RelativeHumidity')
            time_str = st.get('ObsTime', {}).get('DateTime', '')

            if rh_str is None or rh_str == "" or str(rh_str) in ["-99.0", "-999.0", "-99", "-999"]:
                continue
    ...
    def generate_map(self, data):
        ...
        draw.text((25, 25), " 相對濕度即時觀測圖", fill="#ffffff", font=font_title)
        ...
        def get_rh_color(rh):
            min_rh, max_rh = 40.0, 80.0
            mid_rh = (min_rh + max_rh) / 2.0
            rh = max(min_rh, min(max_rh, rh))
            
            if rh <= mid_rh:
                # 40%~60%: 橘色 (255, 165, 0) -> 白色 (255, 255, 255)
                ratio = (rh - min_rh) / (mid_rh - min_rh)
                r = 255
                g = int(165 + 90 * ratio)
                b = int(255 * ratio)
            else:
                # 60%~80%: 白色 (255, 255, 255) -> 天藍色 (0, 195, 255)
                ratio = (rh - mid_rh) / (max_rh - mid_rh)
                r = int(255 * (1 - ratio))
                g = int(255 - 60 * ratio)
                b = 255
            return (r, g, b, 255)
        ...
        for st in stations:
            ...
            rh_str = we.get("RelativeHumidity")
            if rh_str is None or rh_str == "" or str(rh_str) in ["-99.0", "-999.0", "-99", "-999"]:
                continue
                
            try:
                rh_val = float(rh_str)
                if rh_val < 0 or rh_val > 100:
                    continue
            except (ValueError, TypeError):
                continue
        ...
        if hasattr(draw, 'textbbox'):
            tw_80 = draw.textbbox((0, 0), text_80, font=font_legend)[2] - draw.textbbox((0, 0), text_80, font=font_legend)[0]
            tw_60 = draw.textbbox((0, 0), text_60, font=font_legend)[2] - draw.textbbox((0, 0), text_60, font=font_legend)[0]
            tw_40 = draw.textbbox((0, 0), text_40, font=font_legend)[2] - draw.textbbox((0, 0), text_40, font=font_legend)[0]
        else:
            tw_80, _ = draw.textsize(text_80, font=font_legend)
            tw_60, _ = draw.textsize(text_60, font=font_legend)
            tw_40, _ = draw.textsize(text_40, font=font_legend)
            
        draw.text((legend_x - tw_80 - 10, legend_y - 5), text_80, fill="white", font=font_legend)
        draw.text((legend_x - tw_60 - 10, legend_y + legend_h // 2 - 10), text_60, fill="white", font=font_legend)
        draw.text((legend_x - tw_40 - 10, legend_y + legend_h - 15), text_40, fill="white", font=font_legend)
        draw.text((legend_x - 15, legend_y - 25), "%", fill="white", font=font_legend)
    ...
    async def build_embed(self):
        display_results = self.parsed_results[:]
        display_results.sort(key=lambda x: x['rh'], reverse=self.is_high)
        display_results = display_results[:10]

        message_content = "💧 現在最高相對濕度排行" if self.is_high else "💧 現在最低相對濕度排行"
        ...
        for i, r in enumerate(display_results):
            ...
            if i < 3:
                rank_str = ['`🥇`', '`🥈`', '`🥉`'][i]
                line = f"{num_emoji} `{icon} {rh_val:g} %` **{r['county']}{r['town']}** {rank_str}"
            else:
                line = f"{num_emoji} `{icon} {rh_val:g} %` **{r['county']}{r['town']}**"
        ...
        if self.show_image:
            ...
            if self.cached_image:
                file = discord.File(io.BytesIO(self.cached_image), filename="rh_map.png")
                embed.set_image(url="attachment://rh_map.png")
            else:
                embed.description += "\n\n❌ **目前無法取得相對濕度觀測圖**"
...
class RelativeHumidityCog(commands.Cog):
    ...
    async def fetch_rh_data(self):
        url = f"https://opendata.cwa.gov.tw/api/v1/rest/datastore/O-A0001-001?Authorization={self.api_key}&WeatherElement=RelativeHumidity"
        try:
    ...
    @app_commands.choices(濕度類型=[
        app_commands.Choice(name="最高濕度", value="high"),
        app_commands.Choice(name="最低濕度", value="low")
    ], 濕度圖=[
        app_commands.Choice(name="顯示", value="yes"),
        app_commands.Choice(name="不顯示", value="no")
    ])
    async def rh_command(self, interaction: discord.Interaction, 濕度類型: app_commands.Choice[str] = None, 濕度圖: app_commands.Choice[str] = None):
        if not self.api_key:
        ...
        try:
            ...
            stations = data.get('records', {}).get('Station', [])
            if not stations:
                self.fetch_rh_data.invalidate_all()
                await interaction.followup.send("⚠️ 找不到有效的相對濕度資料。")
                return

            is_high = True
            if 濕度類型 and 濕度類型.value == 'low':
                is_high = False
        ...
        for row in message.components:
            for child in row.children:
                if getattr(child, "type", None) == discord.ComponentType.select:
                    ...
                    if child.label == "隱藏濕度圖": show_image = True
                    if child.label == "隱藏詳細資訊": show_details = True
                    
        view = RHView(self.bot, data, interaction.user.id, is_high, show_image)
...
async def setup(bot):
    await bot.add_cog(RelativeHumidityCog(bot))